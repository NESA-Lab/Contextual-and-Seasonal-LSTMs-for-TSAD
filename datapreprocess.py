...
def complete_timestamp(timestamp, arrays=None):
    ...
    src_indevalue = np.argsort(timestamp)
    timestamp_sorted = timestamp[src_indevalue]
...
def standardize_kpi(values, mean=None, std=None, evaluecludes=None):
    """
    Standardize a
    Args:
        ...
        std (float): If not :obj:`None`, will use this `std` to standardize
            ...
            Note `mean` and `std` must be both :obj:`None` or not :obj:`None`.
            (default :obj:`None`)
        evaluecludes (np.ndarray): Optional, 1-D `int32` or `bool` array, the
            indicators of whether each point should be evaluecluded for computing
    ...
    if evaluecludes is not None:
        evaluecludes = np.asarray(evaluecludes, dtype=np.bool)
        if evaluecludes.shape != values.shape:
            raise ValueError(
                "The shape of `evaluecludes` does not agree with "
                "the shape of `values` ({} vs {})".format(evaluecludes.shape, values.shape)
            )

    if mean is None:
        if evaluecludes is not None:
            val = values[np.logical_not(evaluecludes)]
        else:
            val = values
        mean = val.mean()
        std = val.std()
...
def add_attributes(df):
    ...
    #相对差分
    # df['relative_diff']=df['diff_1']/df['diff_1'].rolling(window=5, min_periods=1).apply(lambda value: value.abs().sum())
    # df['relative_diff_2']=df['diff_2']/df['diff_2'].rolling(window=5, min_periods=1).apply(lambda value: value.abs().sum())
    # df=df.drop(['diff_1','diff_2'],avalueis=1)
    # 最大值、最小值、均值、中位数、标准差、偏度、峰度
    reference_data=df['value'].rolling(window=24,min_periods=1)
    df['max']=df['value'].max()
    df['min']=df['value'].min()
    # df['rolling_mavalue'] = df['value'].rolling(window=3,min_periods=1).mavalue()
    # df['rolling_min'] = df['value'].rolling(window=3,min_periods=1).min()
    df['rolling_mean'] = reference_data.mean()
    df['rolling_median'] = reference_data.median()
    df['rolling_std'] = reference_data.std()
    # df['rolling_skew'] = reference_data.skew()
    # df['rolling_kurt'] = reference_data.kurt()
        # 变异系数 = 标准差 / 均值
    df['rolling_cv'] = df['rolling_std'] / df['rolling_mean']
    # 计算滚动窗口内的熵
    df['rolling_entropy'] = reference_data.apply(calc_entropy, raw=True)
    #指数移动平均（带衰减）
    df['evaluep_moving_avg'] = df['value'].ewm(span=7, adjust=False,min_periods=1).mean()
    # #周同比
    # df['value_yoy_week'] = df['value'].shift(7*24)
    # df['yoy_week']=(df['value']-df['value_yoy_week'])/df['value_yoy_week']
    #日同比
    # df['value_yoy_day'] = df['value'].shift(24)  # 一天有24小时
    df['value_yoy_day'] = sum(df['value'].shift(i * 24) for i in range(1, 8)) / 7
    df['yoy_day'] = (df['value'] - df['value_yoy_day']) / df['value_yoy_day']
    return df