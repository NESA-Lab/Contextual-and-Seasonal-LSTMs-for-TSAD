...
class AnomalyTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.hour_filter_linear = nn.Sequential(
            nn.Linear(self.hp.d_model//4, self.hp.d_model//4),
            nn.Tanh(),
            nn.Linear(self.hp.d_model//4, self.hp.d_model//4),
            nn.Tanh(),
        ...
        self.day_filter_linear = nn.Sequential(
            nn.Linear(self.hp.d_model, self.hp.d_model),
            nn.Tanh(),
            nn.Linear(self.hp.d_model, self.hp.d_model),
            nn.Tanh(),
    ...
    def encode(self,input):
        ...
        hour_filter = self.hour_filter(result_hour)
        ...
        day_filter = self.day_filter(result_day)


        #特征融合
        return day_mu, day_log_var, hour_mu, hour_log_var, day_filter, hour_filter
    ...
    def loss_func(self, input, input_normal, mask):
        day_mu, day_log_var, hour_mu, hour_log_var, day_filter, hour_filter = self.encode(input)
        ...
        hour_filter = hour_filter.squeeze(1)
        day_mu = day_mu.squeeze(1)
        day_log_var = day_log_var.squeeze(1)
        day_filter = day_filter.squeeze(1)
        ...
        hour_filter_loss = torch.mean(
            torch.mean(torch.max(torch.zeros_like(mask_hour), (hour_log_var + (input_hour - hour_mu)**2/torch.exp(hour_log_var) - hour_filter)*mask_hour) + 
            torch.max(torch.zeros_like(mask_hour), (hour_filter - hour_log_var - (input_hour - hour_mu)**2/torch.exp(hour_log_var))*torch.logical_not(mask_hour)),dim=-1),
            dim = 0
        )
        ...
        day_filter_loss = torch.mean(
            torch.mean(torch.max(torch.zeros_like(mask_day), (day_log_var + (input_day - day_mu)**2/torch.exp(day_log_var) - day_filter)*mask_day) + 
            torch.max(torch.zeros_like(mask_day), (day_filter - day_log_var - (input_day - day_mu)**2/torch.exp(day_log_var))*torch.logical_not(mask_day)),dim=-1),
            dim = 0
        )


        loss = hour_recon_loss + day_recon_loss + 5*day_filter_loss + 5*hour_filter_loss
    ...
    def MCMC2(self,input, input_normal):
        ...
        hour_filter = hour_filter[:,:,-1].unsqueeze(2)

        day_loss = (day_log_var + (input_day - day_mu)**2/torch.exp(day_log_var))
        day_loss = day_loss[:,:,-1].unsqueeze(2)
        day_filter = day_filter[:,:,-1].unsqueeze(2)

        co_filter = hour_filter + day_filter
        

        loss +=  hour_loss + day_loss

        # loss /= input[:,:,-1].unsqueeze(1)+1e-9
        recon_x = ((hour_mu[:,:,-1] + day_mu[:,:,-1])/2).unsqueeze(2)
        return loss, recon_x, co_filter
