...
import data_augment
from get_f1_score import best_f1, delay_f1, best_f1_without_pointadjust, event_f1, best_f1_thres
from sklearn.metrics import roc_auc_score
import json
import math
...
class MyATF(LightningModule):
    ...
    def forward(self, x, x_normal, mode, mask):
        ...
        return self.atf.forward(x, x_normal, mode, mask)
    ...
    def test_step(self, data_batch, batch_idx):
        ...
        mask = torch.logical_not(torch.logical_or(y_all, z_all))
        with torch.no_grad():
            recon_prob, recon_x, day_ub, day_lb, hour_ub, hour_lb = self.forward(x, x,"test", mask)
    ...
    def test_epoch_end(self, outputs):
        ...
        day_ub = torch.cat(([x["day_ub"] for x in outputs]), 0)
        day_lb = torch.cat(([x["day_lb"] for x in outputs]), 0)
        hour_ub = torch.cat(([x["hour_ub"] for x in outputs]), 0)
        hour_lb = torch.cat(([x["hour_lb"] for x in outputs]), 0)
        ...
        df["day_ub"] = day_ub.cpu().numpy().reshape(-1)
        df["day_lb"] = day_lb.cpu().numpy().reshape(-1)
        df["hour_ub"] = hour_ub.cpu().numpy().reshape(-1)
        df["hour_lb"] = hour_lb.cpu().numpy().reshape(-1)      
        ...
        # results = {
        #     "auc": auc,
        #     "best_f1": best_f1_socre, 
        #     "best_precision": best_precison,
        #     "best_recall": best_recall,
        #     "delay_f1": delay_f1_score,
        #     "delay_precision": delay_precison,
        #     "delay_recall": delay_recall
        # }
        # if not math.isnan(auc):
        #     self.json_list.append(results)
    ...
    def add_model_specific_args():
        ...
        parser.add_argument("--condition_dim", default = 64, type=int)
        ...
        parser.add_argument("--seg_ano_rate", default = 0.005, type=float)
        ...
        parser.add_argument(
            "--ckpt_path", default="/home/hwces/zlp/FCVAE/ckpt/Yahoo-v1.ckpt", type=str
        )
        return parser
