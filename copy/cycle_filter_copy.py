...
from cycle_model import CycleVAE, Filter_net
import pywt
...
class AnomalyTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.filter_net = Filter_net(5,1,hp)
        ...
        # #生成掩码
        # src_mask = self.generate_causal_mask(9).to(self.hp.gpu)
        ...
        self.position_embedding = nn.Parameter(torch.zeros(6 ,600, self.hp.d_model))
    ...
    def get_input(self, input):
        ...
        hour_input = hour_input[:,:,-self.hp.input_dim:]
    ...
    def encode(self,input):
        ...
        # 计算 hour result
        hour_input = self.filter_net(hour_input).permute(1,0,2)
        f_hour_input = torch.fft.rfft(hour_input,dim=-1)
        ...
        result = self.hour_encoder(hour_input)
        result = result[-1,:,:].unsqueeze(0).permute(1,0,2)
        result = self.hour_linear(result)
